...
export const LevelSelect = ({ onSelectLevel, onBack }: LevelSelectProps) => {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[image:var(--gradient-game-bg)] relative">
      {/* Back Button - desktop: absolute, mobile: inline flow */}
      {onBack && (
        <>
          ...
          <motion.button
            ...
            onClick={onBack}
            className="hidden md:flex absolute top-6 left-6 items-center gap-2 px-4 py-2 rounded-xl bg-secondary/50 hover:bg-secondary text-foreground transition-colors"
          >
          ...
          <motion.button
            ...
            animate={{ opacity: 1, x: 0 }}
            onClick={onBack}
            className="md:hidden self-start mb-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-secondary/50 hover:bg-secondary text-foreground transition-colors"
          >
            <ArrowLeft className="w-4 h-4" />
            <span className="text-sm font-medium">Back</span>
          </motion.button>
      ...
      >
        ...
        <p className="text-lg md:text-xl text-muted-foreground max-w-md mx-auto">
          Master German articles by dragging words to{' '}
          <span className="text-der font-semibold">der</span>,{' '}
      ...
      >
        {levels.map((level) => {
          ...
          return (
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-br ${info.color} opacity-0 group-hover:opacity-20 transition-opacity duration-300 rounded-2xl`}
              />

              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                  <span
                    className={`text-3xl md:text-4xl font-bold bg-gradient-to-r ${info.color} bg-clip-text text-transparent`}
                  >
                    {info.name}
                  </span>
                </div>
